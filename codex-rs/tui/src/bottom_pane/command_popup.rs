...
mod tests {
    ...
    fn collab_command_hidden_when_collaboration_modes_disabled() {
        ...
        assert!(
            !cmds.contains(&"plan-model"),
            "expected '/plan-model' to be hidden when collaboration modes are disabled, got {cmds:?}"
        );
    }
    ...
    #[test]
    fn plan_model_command_visible_when_collaboration_modes_enabled() {
        let mut popup = CommandPopup::new(
            Vec::new(),
            CommandPopupFlags {
                collaboration_modes_enabled: true,
        ...
        popup.on_composer_text_change("/plan-model".to_string());

        match popup.selected_item() {
            Some(CommandItem::Builtin(cmd)) => assert_eq!(cmd.command(), "plan-model"),
            other => panic!("expected plan-model to be selected for exact match, got {other:?}"),
        }
    }
